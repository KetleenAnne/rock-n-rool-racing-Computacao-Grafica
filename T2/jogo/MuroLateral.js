...
// Textura para os muros laterais
const texMuro = textureLoader.load("assets/texturas/pista/concreto.jpg");
texMuro.colorSpace = THREE.SRGBColorSpace;
texMuro.wrapS = texMuro.wrapT = THREE.RepeatWrapping;
texMuro.repeat.set(2, 4);

// ========== MATERIAL DO MURO ==========
const materialMuro = new THREE.MeshLambertMaterial({
...
function criarMuroHorizontal(x, z) {
  const muro = new THREE.Mesh(muroGeometryHorizontal, materialMuro);
...
function criarMuroVertical(x, z, altura = null) {
  ...
  const muro = new THREE.Mesh(geometry, materialMuro);
  muro.position.set(x, POSICAO_Y_MURO -0.8, z);
  muro.castShadow = true;
  muro.receiveShadow = true;
  return muro;
...
export function criarMurosPista1(scene) {
  ...
  for (let x = -90; x <= 90; x += 20) {
    ...
    if (x >= -70 && x <= 70) {
      const muro = criarMuroHorizontal(x, 90);
      scene.add(muro);
      muros.push(muro);
    }

    // Sul Interior (z = 110): de -90 a 90
    const muroSul = criarMuroHorizontal(x, 110);
    scene.add(muroSul);
    muros.push(muroSul);

    // Norte Interno (z = -90): de -70 a 70
    if (x >= -70 && x <= 70) {
      const muroNorte = criarMuroHorizontal(x, -90);
      scene.add(muroNorte);
      muros.push(muroNorte);
    }

    // Norte Externo (z = -110): de -90 a 90
    const muroNorteExt = criarMuroHorizontal(x, -110);
  ...
  for (let z = -80; z <= 80; z += 20) {
    ...
    const muroIntEsq = criarMuroVertical(-80, z);
    ...
    const muroIntDir = criarMuroVertical(80, z);
  ...
  for (let z = -100; z <= 100; z += 20) {
    const muroExtEsq = criarMuroVertical(-100, z);
  ...
  for (let z = -100; z <= 100; z += 20) {
    const muroExtDir = criarMuroVertical(100, z);
  ...
  for (let ponta of pontas) {
    const muroPonta = criarMuroVertical(ponta.x, ponta.z);
    scene.add(muroPonta);
...
export function criarMurosPista2(scene) {
  ...
  for (let x of murosSulExt) {
    const muro = criarMuroHorizontal(x * 20 + offsetX, 30 + offsetZ);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let m of murosSulInt) {
    const muro = criarMuroHorizontal(m.x * 20 + offsetX, m.z * 20 + offsetZ);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  const murosNorteExt = [
    { x: 2, z: -9.5 }, { x: 4, z: -9.5 }, { x: 6, z: -9.5 },
    { x: 8, z: -3.5 }, { x: 10, z: -3.5 },
    { x: 1, z: -9.5 }, { x: 3, z: -9.5 }, { x: 5, z: -9.5 },
  ];
  ...
  for (let z of lateraisEsqInt) {
    const muro = criarMuroVertical(30 + offsetX, z * 20 + offsetZ);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let z of lateraisDirInt) {
    const muro = criarMuroVertical(110 + offsetX, z * 20 + offsetZ);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let z of lateraisDirExt) {
    const muro = criarMuroVertical(130 + offsetX, z * 20 + offsetZ);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let z of lateraisLInt) {
    const muro = criarMuroVertical(190 + offsetX, z * 20 + offsetZ);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let z of lateraisLExt) {
    const muro = criarMuroVertical(210 + offsetX, z * 20 + offsetZ);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let z of lateraisEsqExt) {
    const muro = criarMuroVertical(10 + offsetX, z * 20 + offsetZ);
    scene.add(muro);
    muros.push(muro);
  }
...
export function criarMurosPista3(scene) {
  ...
  for (let x of murosSulInt1) {
    const muro = criarMuroHorizontal(x, 50);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let x of murosSulExt1) {
    const muro = criarMuroHorizontal(x, 70);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let x of murosNorteInt1) {
    const muro = criarMuroHorizontal(x, -49.5);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let x of murosNorteExt1) {
    const muro = criarMuroHorizontal(x, -70.5);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let x of murosNorteExt2) {
    const muro = criarMuroHorizontal(x, -190.5);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let x of murosNorteInt2) {
    const muro = criarMuroHorizontal(x, -170);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let z of lateraisEsqInt1) {
    const muro = criarMuroVertical(-39.5, z);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let z of lateraisDirExt1) {
    const muro = criarMuroVertical(60.5, z);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  const laterais2 = [
    ...
    { x: -39.5, z: -160 }, { x: -39.5, z: -140 },
    { x: -39.5, z: -120 }, { x: -39.5, z: -100 },
    { x: -39.5, z: -80 },

    // LATERAL INTERIOR DIREITO SEGUNDO
    { x: -60.5, z: -140 }, { x: -60.5, z: -120 },
    { x: -60.5, z: -100 }, { x: -60.5, z: -80 },

    // LATERAL INTERIOR ESQUERDO SEGUNDO
    { x: -139.5, z: -140 }, { x: -139.5, z: -120 },
    { x: -139.5, z: -100 }, { x: -139.5, z: -80 },

    // LATERAL EXTERIOR ESQUERDO SEGUNDO
    { x: -160.5, z: -160 }, { x: -160.5, z: -140 },
    { x: -160.5, z: -120 }, { x: -160.5, z: -100 },
    { x: -160.5, z: -80 },
  ];

  for (let m of laterais2) {
    const muro = criarMuroVertical(m.x, m.z);
    scene.add(muro);
    muros.push(muro);
  }
  ...
  for (let p of pontasEspeciais) {
    const muro = criarMuroVertical(p.x, p.z, p.w);
    scene.add(muro);
    muros.push(muro);
  }
