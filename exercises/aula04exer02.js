...
function moveSpheres() {
  if (animateSphere1) {
    tranlateSphere(sphere1, finalPosition1, 0.05);
  }
  if (animateSphere2) {
    tranlateSphere(sphere2, finalPosition2, 0.01);
  }
}

function tranlateSphere(sphere, destination, alpha) {
  //move dont use lerp, using translate
  //sphere.position.y;
  while (sphere.positionZ <= destination[2])
    //if (sphere.positionZ != destination[2]) {
    sphere.translateZ(-alpha);
  //}
}
