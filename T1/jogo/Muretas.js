...
export function criarMuretasPista1(scene) {
  ...
  for (let x = -45; x <= 45; x += 10) {
    ...
    material =
      ((x + 45) / 10) % 2 === 0 ? materialMuretaVermelha : materialMuretaBranca;
    ...
    material =
      ((x + 35) / 10) % 2 === 0 ? materialMuretaVermelha : materialMuretaBranca;
...
export function criarLinhaLargada(x, z) {
  ...
  const quadradoGeometry = new THREE.BoxGeometry(
    tamanhoQuadrado,
    0.01,
    tamanhoQuadrado
  );
  ...
  for (let i = 0; i < numQuadrados; i++) {
    for (let j = 0; j < numQuadrados; j++) {
      const material =
        (i + j) % 2 === 0 ? materialQuadradoBranco : materialQuadradoPreto;
